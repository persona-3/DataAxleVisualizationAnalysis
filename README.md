...
## FullContact Analysis

The **FullContact** pipeline generates the same style of dashboards from the `fullcontact_matches` table. Outputs use the `fullcontact_` prefix (e.g. `fullcontact_summary_dashboard.png`, `fullcontact_user_dashboard.html`).

### Config (env or CLI)

You can configure table name, database URL, and column names via **environment variables** (e.g. in `.env`) or **CLI arguments**:

| Purpose | Env variable | CLI argument | Default |
|--------|----------------|--------------|---------|
| PostgreSQL URL | `FULLCONTACT_DATABASE_URL` or `DATABASE_URL` | `--postgres` | (required) |
| Table name | `FULLCONTACT_TABLE` | `--table` | `fullcontact_matches` |
| Store ID column (per-store dashboards) | `FULLCONTACT_STORE_COLUMN` | `--store-col` | `external_store_id` |
| Email column | `FULLCONTACT_EMAIL_COLUMN` | `--email-col` | `email` |
| JSON data column | `FULLCONTACT_DATA_COLUMN` | `--data-col` | `response_json` |

**Example `.env` for FullContact:**

```bash
# FullContact pipeline (optional; falls back to DATABASE_URL)
FULLCONTACT_DATABASE_URL=postgresql://user:password@host:5432/dbname
FULLCONTACT_TABLE=fullcontact_matches
FULLCONTACT_STORE_COLUMN=external_store_id
FULLCONTACT_EMAIL_COLUMN=email
FULLCONTACT_DATA_COLUMN=response_json
```

**Run FullContact dashboard:**

```bash
# Use .env (FULLCONTACT_* or DATABASE_URL)
python3 fullcontact_dashboard.py

# Or override via CLI
python3 fullcontact_dashboard.py --postgres "postgresql://..." --table fullcontact_matches --store-col external_store_id
```
